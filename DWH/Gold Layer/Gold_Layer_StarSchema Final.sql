...
/* ── 2.2  DIM_PLATFORM ───────────────────────────────────── */
-- NOTE: silver.platform_survey has no platform_category column.
--       platform_category is derived here using a CASE expression.
...
/* ── 2.3  DIM_CONTENT ────────────────────────────────────── */
-- Grain: one row per individual content type (already exploded in platform_survey)
-- e.g. "Comedy & Entertainment", "Religious", "Sports" ...
...
-- Source: silver.reels_survey (one row per respondent)
...
-- FIX: platform_survey has NO platform_category column —
--      category is derived here with CASE.
...
WHERE NOT EXISTS (
    ...
    WHERE dp.platform_name = distinct_platforms.platform_name
);
GO


/* ── 4.3  Load gold.DIM_CONTENT ──────────────────────────── */
-- Source: silver.platform_survey
-- FIX: content_type here is already EXPLODED (one value per row),
--      e.g. "Comedy & Entertainment", "Religious" — NOT a combined string.
--      So we load each distinct individual content type as one dimension row.
...
WHERE NOT EXISTS (
    ...
    WHERE dc.content_type = distinct_content.content_type
);
GO


/* ── 4.4  Load gold.DIM_BEHAVIOR ─────────────────────────── */
-- Source: silver.reels_survey — one behavioral profile per respondent
...
/* ─────────────────────────────────────────────────────────────
   ...
   Grain : one row per (respondent × platform × content type)
   Source: silver.platform_survey  joined to  silver.reels_survey
   FIX   : JOIN on the exploded content_type from platform_survey
            directly to gold.DIM_CONTENT — no string-combination mismatch.
   ───────────────────────────────────────────────────────────── */
...
SELECT
    ...
    -- Measures from silver.reels_survey
...
-- Resolve platform FK — match on exploded platform_name
INNER JOIN gold.DIM_PLATFORM AS dp
    ON dp.platform_name = LTRIM(RTRIM(p.primary_platform))

-- Resolve content FK — match on exploded individual content_type
INNER JOIN gold.DIM_CONTENT AS dc
    ON dc.content_type = LTRIM(RTRIM(p.content_type))

-- Bring in respondent-level measures from silver.reels_survey
...
-- Ensure respondent was loaded into the dimension
